...
void Config::Read()
{
    ...
    if (!iniFile)
    {
        spdlog::error("CONFIG ERROR: File not found: {}", pConfigFile.string());
        ...
        std::cout << "ERROR: File not found: " << pConfigFile.string() << std::endl;
...
/*        if (Util::IsSteamOS())
        {
            ...
            spdlog::error("If you require further assistance, you can find our support channel at the Enhanced Spliter Cell Discord: {}", DISCORD_URL);
        }*/
    ...
    ConfigHelper::getValue(ini, "Echelon.EchelonGameInfo", ConfigKeys::VerboseLogging_Setting, g_DistanceCulling.isEnabled);
    LOG_CONFIG(ConfigKeys::VerboseLogging_Section, ConfigKeys::VerboseLogging_Setting, g_DistanceCulling.isEnabled);

    ConfigHelper::getValue(ini, "Echelon.EchelonGameInfo", ConfigKeys::VerboseLogging_Setting, g_IntroSkip.isEnabled);
    LOG_CONFIG(ConfigKeys::VerboseLogging_Section, ConfigKeys::VerboseLogging_Setting, g_IntroSkip.isEnabled);

    ConfigHelper::getValue(ini, "Echelon.EchelonGameInfo", ConfigKeys::VerboseLogging_Setting, g_PauseOnFocusLoss.shouldPause);
    LOG_CONFIG(ConfigKeys::VerboseLogging_Section, ConfigKeys::VerboseLogging_Setting, g_PauseOnFocusLoss.shouldPause);

    ConfigHelper::getValue(ini, "Echelon.EchelonGameInfo", ConfigKeys::VerboseLogging_Setting, bShouldCheckForUpdates);
    LOG_CONFIG(ConfigKeys::VerboseLogging_Section, ConfigKeys::VerboseLogging_Setting, g_Logging.bVerboseLogging);

    ConfigHelper::getValue(ini, "Echelon.EchelonGameInfo", ConfigKeys::VerboseLogging_Setting, bConsoleUpdateNotifications);
    LOG_CONFIG(ConfigKeys::VerboseLogging_Section, ConfigKeys::VerboseLogging_Setting, g_Logging.bVerboseLogging);
